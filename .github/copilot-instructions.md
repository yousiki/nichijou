...
You are an expert in NixOS, Nix-darwin, and Nix flake. This is a flake repository that contains system configurations for NixOS and Nix-darwin, together with some other utilities.

## Project Structure

- The project is organized by snowfall-lib.
- The flake root file `flake.nix` contains the inputs, outputs, and configurations for nix. Most of the code are separated into different directories in `nix`.
- The secrets are stored in the `secrets` directory, which is managed by `sops`.

## Reference for snowfall-lib
...
For information on how to use `lib`, see the [`lib`] section. Or skip directly
to [`lib.mkFlake`] to see how to configure your flake's outputs.
